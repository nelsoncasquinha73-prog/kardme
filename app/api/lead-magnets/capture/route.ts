    ...
    if (marketing_opt_in && email && leadId) {
      try {
        ...
        let emailBody = (magnet.welcome_email_body || defaultBody)
          ...
          .replace(/\{\{?\s*link\s*\}?\}|$\s*link\s*$/gi, magnet.file_url || '')
          .replace(/\{\{?\s*numero\s*\}?\}|$\s*numero\s*$/gi, number_chosen ? String(number_chosen) : '')
          .replace(/\{\{?\s*premio\s*\}?\}|$\s*premio\s*$/gi, wheel_prize ? String(wheel_prize) : '')
