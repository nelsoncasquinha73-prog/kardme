...
import { generateTheme, type Theme } from './generateTheme'
 ...
 */
export function getTheme(theme?: Partial<Theme> | null): Theme {
  // 🎯 A cor base é SEMPRE a primary (ou default)
  const basePrimary = theme?.primary ?? DEFAULT_PRIMARY
  ...
  return {
    ...
    background: theme?.background ?? auto.background,
    surface: theme?.surface ?? auto.surface,
    text: theme?.text ?? auto.text,
    mutedText: theme?.mutedText ?? auto.mutedText,
  }
}
