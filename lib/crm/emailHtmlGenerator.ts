// Gera HTML "email-safe" a partir dos blocos do editor
export function generateEmailHtml(blocks: any[], title: string, subject: string, preheader: string): string {
...
function renderEmailBlockToHtml(block: any): string {
  ...
  switch (type) {
    ...
    case 'video':
      ...
      // Detectar se é YouTube/Vimeo ou upload
      ...
      return `<div style="width: ${content.width || '100%'}; margin: 0 auto 16px; text-align: ${content.align || 'center'};">
        <a href="${escapeHtml(videoLink)}" style="text-decoration: none; display: inline-block; width: 100%; max-width: 500px;">
          <div style="position: relative; display: inline-block; width: 100%; max-width: 500px; background-color: #000; border-radius: 8px; overflow: hidden;">
            ...
            <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 60px; height: 60px; background-color: rgba(16, 185, 129, 0.9); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 28px; text-decoration: none; color: white; box-shadow: 0 4px 12px rgba(16, 185, 129, 0.4);">▶️</div>
          </div>
        </a>
        ${isYoutubeOrVimeo ? `<div style="text-align: center; margin-top: 12px;"><a href="${escapeHtml(videoLink)}" style="display: inline-block; padding: 10px 20px; background-color: #10b981; color: white; text-decoration: none; border-radius: 6px; font-weight: 700; font-size: 14px;">Ver vídeo</a></div>` : ''}
      </div>`
