    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {active && (
        <CollapsibleSection title={`✏️ Decoração #${decorations.findIndex(d => d.id === active.id) + 1}`} subtitle="Imagem, posição, tamanho" isOpen={activeSection === 'edit'} onToggle={() => setActiveSection(activeSection === 'edit' ? null : 'edit')}>
          ...
          <Row label="X (%)"><input type="range" min={0} max={100} step="0.2" value={active.x} onChange={(e) => patchDecoration(active.id, { x: Number(e.target.value) })} style={{ flex: 1 }} /><span style={rightNum}>{active.x}%</span></Row>
          <Row label="Y (%)"><input type="range" min={0} max={100} step="0.2" value={active.y} onChange={(e) => patchDecoration(active.id, { y: Number(e.target.value) })} style={{ flex: 1 }} /><span style={rightNum}>{active.y}%</span></Row>
