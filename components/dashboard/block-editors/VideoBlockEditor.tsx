...
export default function VideoBlockEditor({ cardId, settings, style, onChangeSettings, onChangeStyle }: Props) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploadError(null)
    try {
      const { publicUrl, thumbnailUrl } = await uploadCardVideo({ cardId, file })
      setSettings({ url: publicUrl, thumbnailUrl: thumbnailUrl || undefined })
    } catch (err: any) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <CollapsibleSection title="🎬 Vídeo" subtitle="Upload ou URL" isOpen={activeSection === 'video'} onToggle={() => setActiveSection(activeSection === 'video' ? null : 'video')}>
        ...
        {uploading && <div style={{ fontSize: 11, color: '#3b82f6' }}>⏳ A enviar vídeo e gerar thumbnail...</div>}
        ...
        {settings.thumbnailUrl && <div style={{ fontSize: 10, color: '#22c55e' }}>✓ Thumbnail personalizada</div>}
