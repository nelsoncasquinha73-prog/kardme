    ...
    try {
      ...
      const uploadFile = await (async () => {
        try {
          ...
          // Garantir que é File (algumas versões podem devolver Blob)
          if (compressed instanceof File) return compressed;
          return new File([compressed], file.name, {
            type: file.type || "image/jpeg",
          });
        } catch (e) {
          // fallback: se falhar compressão, faz upload do original
          return file;
        }
      })();
  ...
  const onFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (!url) {
          alert("Erro ao enviar uma das imagens.");
          continue;
        }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      >
        ...
        {uploading && (
          <div style={{ fontSize: 12, opacity: 0.7 }}>A enviar imagens…</div>
        )}
          ...
          >
            ...
            <Row label="Legenda">
              <input
                ...
                value={it.caption ?? ""}
                onChange={(e) =>
                  updateItem(it.uid, { caption: e.target.value })
                }
            ...
            >
              ...
              {(it?.action?.type === "external" ||
                ...
                <>
                  <Row label="Botão">
                    <Toggle
                      active={it?.action?.buttonEnabled ?? false}
                      onClick={() =>
                        updateItem(it.uid, {
                          action: {
                            ...(it?.action || {}),
                            buttonEnabled: !(
                              it?.action?.buttonEnabled ?? false
                            ),
                          },
                        })
                      }
                  ...
                  {it?.action?.buttonEnabled && (
                    <Row label="Texto botão">
                      <input
                        type="text"
                        value={it?.action?.buttonText ?? "Ver mais"}
                        onChange={(e) =>
                          updateItem(it.uid, {
                            action: {
                              ...(it?.action || {}),
              ...
              {it?.action?.type === "external" && (
                <>
                  ...
                  <Row label="Nova aba">
                    <Toggle
                      active={it?.action?.newTab !== false}
                      onClick={() =>
                        updateItem(it.uid, {
                          action: {
                            ...(it?.action || {}),
                            newTab: !(it?.action?.newTab !== false),
                          },
                        })
                      }
                    />
              ...
              {it?.action?.type === "modal" && (
                <>
                  ...
                  <Row label="Vídeo">
                    <select
                      value={it?.action?.videoType ?? "youtube"}
                      onChange={(e) =>
                        updateItem(it.uid, {
                          action: {
                            ...(it?.action || {}),
                  ...
                  {(it?.action?.videoType === "youtube" ||
                    it?.action?.videoType === "vimeo") && (
                    <Row label="URL vídeo">
                      <input
                        type="text"
                        value={it?.action?.videoUrl ?? ""}
                        onChange={(e) =>
                          updateItem(it.uid, {
                            action: {
                              ...(it?.action || {}),
                        ...
                        <input
                          ...
                          onChange={async (e) => {
                            ...
                            if (file.size > MAX_BYTES) {
                              alert(
                                `Vídeo demasiado grande. Máximo: ${MAX_MB}MB.`,
                              );
                              e.currentTarget.value = "";
                              return;
                            }
                  ...
                  >
                    ...
                    <textarea
                      value={it?.action?.modalHtml ?? ""}
                      onChange={(e) =>
                        updateItem(it.uid, {
                          action: {
                            ...(it?.action || {}),
                            modalHtml: e.target.value,
                          },
                        })
                      }
                      onBlur={() => onBlurFlushSave?.()}
                      placeholder="<p>Descrição...</p>"
                      style={{
                        ...inputStyle,
                        minHeight: 80,
                        fontFamily: "monospace",
      ...
      >
        <Row label="Texto">
          <input
            ...
            onChange={(e) =>
              onChangeSettings({ ...s, heading: e.target.value })
            }
            onBlur={() => onBlurFlushSave?.()}
            placeholder="Galeria"
            style={inputStyle}
        ...
        <Row label="Alinhamento">
          <div style={{ display: "flex", gap: 6 }}>
            {(["left", "center", "right"] as const).map((a) => (
              <MiniButton
                key={a}
                active={(st.headingAlign ?? "left") === a}
                onClick={() => setStyle({ headingAlign: a })}
              >
                {a === "left" ? "◀" : a === "center" ? "●" : "▶"}
              </MiniButton>
            ))}
        ...
        <Row label="Cor">
          <ColorPickerProUnified
            ...
            onEyedropper={() =>
              pickEyedropper((hex) => setStyle({ headingColor: hex }))
            }
          />
        </Row>
        <Row label="Negrito">
        ...
        <Row label="Fonte">
          <FontPicker
            ...
            onChange={(v) => setStyle({ headingFontFamily: v || "" })}
          />
        </Row>
        <Row label="Peso">
          <select
            ...
            onChange={(e) =>
              setStyle({ headingFontWeight: Number(e.target.value) })
            }
            style={selectStyle}
          >
        ...
        <Row label="Tamanho">
          <input
            ...
            onChange={(e) =>
              setStyle({ headingFontSize: Number(e.target.value) })
            }
            style={{ flex: 1 }}
          />
      ...
      <CollapsibleSection
        title="🎛 Layout"
        subtitle="Modo, tamanhos, espaçamento"
      ...
      >
        <Row label="Modo">
          <div style={{ display: "flex", gap: 6 }}>
            <MiniButton
              active={(layout.containerMode ?? "full") === "full"}
              onClick={() => setLayout({ containerMode: "full" })}
            >
              Full
            </MiniButton>
            <MiniButton
              active={layout.containerMode === "moldura"}
              onClick={() => setLayout({ containerMode: "moldura" })}
            >
              Moldura
            </MiniButton>
            <MiniButton
              active={layout.containerMode === "autoadapter"}
              onClick={() => setLayout({ containerMode: "autoadapter" })}
            >
              Auto
            </MiniButton>
          </div>
        ...
        <Row label="Gap">
          <input
            ...
            onChange={(e) => setLayout({ gapPx: Number(e.target.value) })}
            style={{ flex: 1 }}
          />
        ...
        <Row label="Padding lateral">
          <input
            ...
            min={0}
            max={64}
            value={layout.sidePaddingPx ?? 16}
            onChange={(e) =>
              setLayout({ sidePaddingPx: Number(e.target.value) })
            }
            style={{ flex: 1 }}
          />
        ...
        <Row label="Largura item">
          <input
            ...
            onChange={(e) => setLayout({ itemWidthPx: Number(e.target.value) })}
            style={{ flex: 1 }}
          />
        ...
        <Row label="Altura item">
          <input
            ...
            onChange={(e) =>
              setLayout({ itemHeightPx: Number(e.target.value) })
            }
            style={{ flex: 1 }}
          />
        ...
        <Row label="Object fit">
          <div style={{ display: "flex", gap: 6 }}>
            <MiniButton
              active={(layout.objectFit ?? "cover") === "cover"}
              onClick={() => setLayout({ objectFit: "cover" })}
            >
              Cover
            </MiniButton>
            <MiniButton
              active={layout.objectFit === "contain"}
              onClick={() => setLayout({ objectFit: "contain" })}
            >
              Contain
            </MiniButton>
          </div>
      ...
      <CollapsibleSection
        title="🎠 Carrossel"
        subtitle="Autoplay, setas, pontos"
        isOpen={activeSection === "carousel"}
        onToggle={() =>
          setActiveSection(activeSection === "carousel" ? null : "carousel")
        }
      >
        <Row label="Autoplay">
          <Toggle
            ...
            onClick={() =>
              setLayout({ autoplay: !(layout.autoplay !== false) })
            }
          />
        </Row>
        {layout.autoplay !== false && (
          <Row label="Intervalo">
            <input
              ...
              onChange={(e) =>
                setLayout({ autoplayIntervalMs: Number(e.target.value) })
              }
              style={{ flex: 1 }}
            />
        ...
        <Row label="Mostrar setas">
          <Toggle
            active={layout.showArrows === true}
            onClick={() =>
              setLayout({ showArrows: !(layout.showArrows === true) })
            }
          />
        </Row>
        {layout.showArrows && (
          <Row label="Só desktop">
            <Toggle
              active={layout.arrowsDesktopOnly !== false}
              onClick={() =>
                setLayout({
                  arrowsDesktopOnly: !(layout.arrowsDesktopOnly !== false),
                })
              }
            />
          </Row>
        ...
        <Row label="Mostrar pontos">
          <Toggle
            active={layout.showDots !== false}
            onClick={() =>
              setLayout({ showDots: !(layout.showDots !== false) })
            }
          />
        </Row>
      </CollapsibleSection>
      ...
      {/* ========== CORES CARROSSEL ========== */}
      <CollapsibleSection
        title="🎨 Cores carrossel"
        subtitle="Setas, pontos"
        isOpen={activeSection === "carouselColors"}
        onToggle={() =>
          setActiveSection(
            activeSection === "carouselColors" ? null : "carouselColors",
          )
        }
      >
        <Row label="Fundo setas">
          <ColorPickerProUnified
            ...
            onEyedropper={() =>
              pickEyedropper((hex) => setStyle({ carouselArrowsBg: hex }))
            }
          />
        </Row>
        <Row label="Ícone setas">
          <ColorPickerProUnified
            ...
            onEyedropper={() =>
              pickEyedropper((hex) =>
                setStyle({ carouselArrowsIconColor: hex }),
              )
            }
          />
        </Row>
        <Row label="Pontos inativos">
          <ColorPickerProUnified
            ...
            onEyedropper={() =>
              pickEyedropper((hex) => setStyle({ carouselDotsColor: hex }))
            }
          />
        </Row>
        <Row label="Pontos ativos">
          <ColorPickerProUnified
            ...
            onEyedropper={() =>
              pickEyedropper((hex) =>
                setStyle({ carouselDotsActiveColor: hex }),
              )
            }
          />
        </Row>
      ...
      <CollapsibleSection
        ...
        onToggle={() =>
          setActiveSection(activeSection === "container" ? null : "container")
        }
      >
        <Row label="Fundo">
          <Toggle
            ...
            onClick={() =>
              setContainer({
                enabled: true,
                bgColor: bgEnabled ? "transparent" : "#ffffff",
              })
            }
          />
        </Row>
        {bgEnabled && (
          <Row label="Cor">
            <ColorPickerProUnified
              ...
              onEyedropper={() =>
                pickEyedropper((hex) => setContainer({ bgColor: hex }))
              }
            />
          </Row>
        )}
        <Row label="Sombra">
          <Toggle
            ...
            onClick={() => setContainer({ shadow: !container.shadow })}
          />
        </Row>
        <Row label="Borda">
          <Toggle
          ...
          <>
            <Row label="Espessura">
              <input
                ...
                onChange={(e) =>
                  setContainer({ borderWidth: Number(e.target.value) })
                }
                style={{ flex: 1 }}
              />
            ...
            <Row label="Cor borda">
              <ColorPickerProUnified
                ...
                onEyedropper={() =>
                  pickEyedropper((hex) => setContainer({ borderColor: hex }))
                }
              />
            </Row>
          </>
      ...
      <CollapsibleSection
        title="📍 Posição"
        subtitle="Offset vertical"
        isOpen={activeSection === "position"}
        onToggle={() =>
          setActiveSection(activeSection === "position" ? null : "position")
        }
      >
        <Row label="Offset Y">
          <input
...
function Row({
  ...
  return (
    ...
    >
      <span
        style={{ fontSize: 12, fontWeight: 600, opacity: 0.8, minWidth: 80 }}
      >
        {label}
      </span>
      <div
